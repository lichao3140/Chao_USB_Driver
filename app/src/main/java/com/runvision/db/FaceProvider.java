...
public class FaceProvider {
   ...
   // private TimeCompareUtil timecompare;
    ...
    public User getUserByUserpath(String Imagepath) {
        User user = null;
        if (null != faceHelper) {
            SQLiteDatabase db = faceHelper.getWritableDatabase();
            if (null != db) {
                String sql = "select * from tUser where templateImageID = " + Imagepath;
                Cursor cursor = db.rawQuery( sql, null);
                if (null != cursor && cursor.getCount() > 0) {
                    cursor.moveToFirst();
                    user = new User();
            ...
            if (null != db) {
                ...
                if (null != cursor && cursor.getCount() > 0) {
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        list.add(user);
                        Log.i("zhuhuilong","数据库数据："+cursor.getColumnIndex("id"));
                        Log.i("zhuhuilong","数据库数据："+cursor.getColumnIndex("type"));
...
//        String sql = "select count(id) from tRecord";
    ...
    public List<Atd> queryATDRecord(String sql) {
        List<Atd> list = null;
        if (null != faceHelper) {
            SQLiteDatabase db = faceHelper.getWritableDatabase();
            if (null != db) {
                Cursor cursor = db.rawQuery(sql, null);
                if (null != cursor) {
                    list = new ArrayList<Atd>();
                    while (cursor.moveToNext()) {
                        //timecompare=new TimeCompareUtil();
                        if(AppData.getAppData().getAtd_type().equals("1")) {
                            Atd user = new Atd();
                            user.setId(cursor.getInt(cursor.getColumnIndex("id")));
                            user.setName(cursor.getString(cursor.getColumnIndex("name")));
                            user.setSex(cursor.getString(cursor.getColumnIndex("sex")));
                            user.setAge(cursor.getInt(cursor.getColumnIndex("age")));
                            user.setNumber(cursor.getString(cursor.getColumnIndex("workNo")));
                            user.setIdcard(cursor.getString(cursor.getColumnIndex("cardNo")));
                            user.setType("1");
                            user.setRecordtype(cursor.getString(cursor.getColumnIndex("comperType")));
                            user.setDate(TestDate.timet(cursor.getLong(cursor.getColumnIndex("createTime"))));
                            list.add(user);
                        }
                       else if(AppData.getAppData().getAtd_type().equals("2")) {
                            Atd user = new Atd();
                            user.setId(cursor.getInt(cursor.getColumnIndex("id")));
                            user.setName(cursor.getString(cursor.getColumnIndex("name")));
                            user.setSex(cursor.getString(cursor.getColumnIndex("sex")));
                        ...
                        else if(AppData.getAppData().getAtd_type().equals("0")) {
        ...
        if (null != faceHelper) {
            ...
            if (null != db) {
                ...
                if (null != cursor) {
                    ...
                    while (cursor.moveToNext()) {
                        timecompare=new TimeCompareUtil();
                        if(timecompare.TimeCompare(AppData.getAppData().getAtd_up_startime(),AppData.getAppData().getAtd_up_endtime(),
                                TestDate.timet(cursor.getLong(cursor.getColumnIndex("createTime"))).substring(11, 16))&&
                                AppData.getAppData().getAtd_type().equals("1"))
                        ...
                        else if(timecompare.TimeCompare(AppData.getAppData().getAtd_down_startime(),AppData.getAppData().getAtd_down_endtime(),
                                TestDate.timet(cursor.getLong(cursor.getColumnIndex("createTime"))).substring(11, 16))&&
                                AppData.getAppData().getAtd_type().equals("2"))
                        {
                            Atd user = new Atd();
                        ...
                        else if(timecompare.TimeCompare(AppData.getAppData().getAtd_up_startime(),AppData.getAppData().getAtd_up_endtime(),
                                TestDate.timet(cursor.getLong(cursor.getColumnIndex("createTime"))).substring(11, 16))||
                                (timecompare.TimeCompare(AppData.getAppData().getAtd_down_startime(),AppData.getAppData().getAtd_down_endtime(),
                                TestDate.timet(cursor.getLong(cursor.getColumnIndex("createTime"))).substring(11, 16)))&&
                                        AppData.getAppData().getAtd_type().equals("0"))
                        {
                            Atd user = new Atd();
